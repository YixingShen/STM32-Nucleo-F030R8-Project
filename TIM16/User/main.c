...
#if 0//使用PLL作为系统时钟
	TIM16->PSC|=3;//Set prescaler to 3, so APBCLK/4 i.e 12MHz
	TIM16->ARR=12000-1;//as timer clock is 12MHz, an event occurs each 1ms
#else //使用HSI作为系统时钟
	TIM16->PSC = 0;//HSI=8MHz
	TIM16->ARR = 8000-1;//as timer clock is 8MHz, an event occurs each 1ms	
#endif	
	//TIM16->CR1 &= ~TIM_CR1_DIR;//TIM6 TIM14 TIM15 TIM16计数器只能向上计数，所以没有DIR位
#if 1
	TIM16->DIER |= TIM_DIER_UIE;
	/* Configure NVIC for Timer 15 update event */
	NVIC_EnableIRQ(TIM16_IRQn); // Enable Interrupt
	NVIC_SetPriority(TIM16_IRQn,0); //Set priority for UEV	
#endif	
