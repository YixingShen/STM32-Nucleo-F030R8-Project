...
void TIM_config(void)
{
...
#if 0//使用PLL作为系统时钟
	TIM6->PSC|=3;//Set prescaler to 3, so APBCLK/4 i.e 12MHz
	TIM6->ARR=12000-1;//as timer clock is 12MHz, an event occurs each 1ms
#else //使用HSI作为系统时钟
	TIM6->PSC = 0;//HSI=8MHz
	...
	NVIC_SetPriority(TIM6_IRQn,0); //Set priority for UEV	
#endif	
