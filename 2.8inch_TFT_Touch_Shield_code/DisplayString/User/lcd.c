...
static void lcd_write_word(uint16_t hwData)
{
    LCD_DC_SET;
    LCD_CS_CLR;
    spi_read_write_byte(hwData >> 8);
    spi_read_write_byte(hwData & 0xFF);
    LCD_CS_SET;
}
...
void lcd_clear_screen(uint16_t hwColor)  
{
	...
	LCD_CS_CLR;
	for (i = 0; i < wCount; i ++) {
		spi_read_write_byte(hwColor >> 8);
		spi_read_write_byte(hwColor & 0xFF);
	}
	LCD_CS_SET;
}
